...
// Idempotent by construction: strip any block a previous run appended (the
// marker through EOF — this script owns the tail of the file) before writing
// the fresh one. Without this, a re-run on an already-stamped dist appends a
// SECOND `/*` CSP rule, and Netlify's first-match-wins keeps serving the
// stale first one — the exact silent-white-screen class the CSP smoke guards.
const GENERATED_MARKER = '# Security headers — generated by scripts/csp-hashes.mjs'
const rawExisting = existsSync(headersFile)
  ? readFileSync(headersFile, 'utf8')
  : ''
const markerAt = rawExisting.indexOf(GENERATED_MARKER)
const existingContent = (markerAt === -1 ? rawExisting : rawExisting.slice(0, markerAt)).trimEnd()
