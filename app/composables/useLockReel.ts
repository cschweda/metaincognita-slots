...
export function useLockReel() {
  ...
  function creditsToDollars(credits: number): string {
    ...
    return formatCentsCompact(credits * betCoins.value * d.denominationCents)
  }
  ...
  const collectDollars = computed((): string => {
    const d = def.value
    if (d === null) return '$0'
    return formatCentsCompact(collectCredits.value * betCoins.value * d.denominationCents)
  })
  const betDollars = computed((): string => {
    const d = def.value
    if (d === null) return '$0.00'
    return formatCentsCompact(betCoins.value * d.denominationCents)
  })
  ...
  const betChips = computed(() => {
    ...
    return BET_CHIPS.map(c => ({ coins: c, dollars: formatCentsCompact(c * cents), active: store.currentBet === c }))
  })
  ...
  const resultOutcome = computed((): LockResult | null => {
    ...
    const banked = formatCentsCompact(lr.collectCredits * lr.ante * d.denominationCents)
    if (filled) {
      return {
        kind: 'grand',
        title: 'GRAND!',
