 ...
 * put, every still-empty cell respins, any new lock resets the respins, the 7s
 * turn sticky and upgrade, and filling the whole 20-cell grid awards the GRAND.
 * Two 7s leave a one-reel re-stop "tease" (EV 0, wired by the UI later).
 *
 ...
 * reels), so the strips are long and very blank-heavy to dilute the 20-cell
 * collect down to a sub-1.0 expectation. The big prizes (MAJOR, GRAND) live in
 * the bonus tail, which keeps the base lively without blowing out RTP.
 *
 * Calibrated (a two-phase exact-RTP sweep; the throwaway scripts/_sl-calibrate.ts
 * was deleted after baking the numbers in) to:
 *   - RTP/coin       94.4798%   (target ~94.5%, the floor's slightly-generous band)
 *   - hit frequency  26.12%     (high variance — frequent-ish small collects, a
 *                                rare big bonus; SD ≈ 4.43 credits/coin)
 *   - 777 bonus      ~1 in 96 rounds; bonus EV given trigger ≈ 39.7 credits/coin
 *   - the GRAND (a full 20-cell grid) is a true astronomical lottery (~1 in 270M)
 *     — its RTP contribution is ~5.6e-7 credits/coin, i.e. ~0%; it is showcase, not
 *     a load-bearing payout (the bonus value is the sticky-7 upgrade + extra locks)
 * The exact figure is FROZEN in tests/machines-lockreel.test.ts and cross-checked
 * by the seeded multi-million-round sim inside `pnpm verify`'s 3.5σ band.
 *
 ...
 *
 * ── Strips ─────────────────────────────────────────────────────────────────
 * The five 692-cell virtual strips are produced by a deterministic builder (a
 * long blank-heavy reel is impractical as a literal). `buildStrip` reproduces the
 ...
 * only moves the hit frequency and the bonus rate. The frozen RTP test pins the
...
// Per-reel calibrated specs. Cash is C1-heavy (for the hit frequency) with a
...
// the base a little prize texture, while the GRAND is bonus-fill only.
const REEL_SPECS: ReelSpec[] = [
  { cash: [['C1', 7], ['C2', 2]], sevens: 4, prize: 'MINI' },
  { cash: [['C1', 7], ['C2', 2], ['C3', 1]], sevens: 3, prize: null },
  { cash: [['C1', 6], ['C2', 2], ['C5', 1]], sevens: 3, prize: 'MAJOR' },
  { cash: [['C1', 7], ['C2', 1], ['C3', 1]], sevens: 2, prize: 'MINI' },
  { cash: [['C1', 6], ['C2', 2], ['C5', 1]], sevens: 2, prize: null }
]
...
export const STOP_AND_LOCK_777: LockReelMachineDef = {
  ...
  prizes: { MINI: 5, MAJOR: 15, GRAND: 150 },
  sevenSymbol: 'SEVEN',
  blankSymbol: 'BLANK',
  bonus: { respins: 4, sevenUpgrade: 10, grandOnFill: 'GRAND' },
  progressive: null,
